...
class PatientMatcher():
	...
	def __init__(self, excel_file, **read_xls_args):
		...
		self.var_cols = data.columns.drop('BIPOLAR')
		self.bin_cols = data.columns[data.isin([0, 1]).all()].drop('BIPOLAR').tolist()
	...
	def get_best_matches(self, feature_weights=None, norm_order=2):
		...
		# split into bipolar and control
		self.bipolar = self.data.loc[self.data.BIPOLAR, self.var_cols].sub(
			self.mean).div(self.std)
		self.control = self.data.loc[~self.data.BIPOLAR, self.var_cols].sub(
			self.mean).div(self.std)
		...
		d = pd.Series({
			(control_id, bipolar_id): minkowski(self.control.loc[control_id], self.bipolar.loc[bipolar_id], p=norm_order, w=fw)
			for control_id, bipolar_id in product(self.control.index, self.bipolar.index)
		})
		d.index.names = ['control_id', 'bipolar_id']
		self.dissimilarity_matrix = d.unstack('bipolar_id')
		...
		# a control patient can be matched to no more than one bipolar
		for i in range(x.shape[0]):
			prob += pulp.lpSum(x[i, :]) <= 1

		# each bipolar patient must have exactly one match
	...
	def decomp_viz(self, matches, decomp_method='PCA', random_state=42, **algo_kwargs):
		...
		**algo_kwargs:
		...
		decomp[decomp.index.isin(self.bipolar.index)].plot.scatter(
			'dim1', 'dim2', c='C1', ax=ax[0], label='Bipolar')
		...
		for match in matches.control_id.iteritems():
			ax[0].add_line(
				plt.Line2D(
					xdata=decomp.loc[match, 'dim1'],
					ydata=decomp.loc[match, 'dim2'],
					color='grey',
					alpha=.3
				)
			)
